function [y, MS] = myMeanShift(x,hs,hr,th,plotOn)
...
%% initialization
x = double(x);  % bring the img in double
[height,width,depth] = size(x);
y = x;  % make a copy of the img
done = 0;
iter = 0;
if plotOn   % prepare a figure with a rndm value as title
